...
def drawChest(ax: Axes, x: float, y: float) -> None:
    ...
    ax.add_patch(
        Rectangle(
            ...
            2,
            2,
            facecolor=CHEST_COLOR,
            edgecolor=CHEST_TRIM,
            linewidth=0.5,
            zorder=4,
        )
    )
...
def drawCabin(ax: Axes, x: int, y: int, w: int = 12, h: int = 8) -> None:
    ...
    ax.add_patch(Rectangle((x, y + h - 1), w, 1, facecolor=TILE_COLORS[132], zorder=2))
    ax.add_patch(Rectangle((x, y), w, 1, facecolor=TILE_COLORS[132], zorder=2))
...
def drawPyramid(
    ...
    drawWallFill(ax, x + w // 4, y + h // 2, w // 2, h // 4, color=TILE_COLORS[125], alpha=0.55)
...
def drawFloatingIsland(
    ...
    drawTileGrid(ax, grid)
    drawWallFill(ax, x + w // 3, y + 1, w // 3, h // 2, color=TILE_COLORS[132], alpha=0.6)
...
def drawSpiderCave(
    ...
    ax.scatter(
        ex, ey, s=10, c="#8B0000", marker="o", edgecolors=PALETTE["bg"], linewidths=0.4, zorder=3
    )
...
def drawGemCave(
    ...
    ey = y + rng.integers(0, max(1, h), size=n)
    ax.scatter(
        ex, ey, s=14, c=gemColor, marker="*", edgecolors=PALETTE["fg"], linewidths=0.3, zorder=3
    )
