"""Original pixel renderer and media exporters for TerraForge worlds."""
...
from terraforge.pipeline import TerraForgePipeline
from terraforge.tiles import TILE_STYLES, WALL_COLORS, Biome, Liquid, Tile, Wall
...
def save_generation_gif(
    ...
    *,
    ...
    if config.hardmode:
        milestones.add("Hardmode V Transformation")
    ...
    def capture(spec, world: GeneratedWorld) -> None:
        ...
        frames.append(
            add_title_bar(
                image,
                f"{spec.index:03d}  {spec.name}",
                f"{spec.phase.value} | {spec.fidelity.value}",
            )
        )

    TerraForgePipeline().generate(config, snapshot=capture)
    destination = Path(path)
    destination.parent.mkdir(parents=True, exist_ok=True)
    frames[0].save(
        destination,
